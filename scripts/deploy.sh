...
# Сначала обычная сборка (BuildKit). Если она падает, повторяем классическим
# сборщиком. Зачем: 08.09.2026 на боевом сервере BuildKit не смог достучаться до
# репозитория Alpine («TLS: unspecified error» на apk add openssl), при том что
# и хост, и обычный контейнер тот же адрес открывали без проблем — то есть
# ломается именно сетевое окружение сборщика, а не сеть машины. Классический
# сборщик работает в обычной сети Docker и собирает нормально. Проверка «а вдруг
# уже починилось» стоит одну неудачную попытку, поэтому порядок именно такой.
if ! docker compose up -d --build; then
  echo "==> BuildKit не собрал. Повтор классическим сборщиком"
